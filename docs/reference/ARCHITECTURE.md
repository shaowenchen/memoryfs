...
| 后端 | 说明 |
|------|------|
| `memory` | 纯内存（默认）。reads/writes 直接命中 RAM，无中间缓存层；`storageGB` 设置时按容量预分配 buffer slot |
| `disk` | 直接磁盘 I/O（Direct）。reads/writes 直接落盘，`storageGB` 作为容量上限 |

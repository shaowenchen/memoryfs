...
| Raft 日志大量 `:8081 connection refused` | hostPath 残留旧端口 Raft 成员地址；**各节点**执行 `rm -rf /data/memoryfs/<instanceId>/` 后 `helm uninstall` 再装；或单 Pod 设 `MEMORYFS_RAFT_RESET=true` 仅清 raft 状态（需 quorum 协调） |
